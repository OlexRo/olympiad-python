...
# Выходные данные
# В первую строку выходного файла OUTPUT.TXT нужно вывести числа, которые соответствуют дням месяцев, в которые Вася получил тройки, а во второй строке
# соответственно расположить числа месяца, в которые Вася получил четверки. В третьей строке нужно вывести «YES», если Вася может рассчитывать на четверку и «NO» в
# противном случае. В каждой строчке числа следует выводить в том же порядке, в котором они идут во входных данных. При выводе числа отделяются пробелом.
...
for i in range(len(array)):
  if array[i] % 2 == 0:
    even_days.append(array[i])
  else:
    odd_days.append(array[i])

with open('output.txt', 'w') as f:
  f.write(' '.join(map(str, odd_days)) + '\n')
  f.write(' '.join(map(str, even_days)) + '\n')
  if len(even_days) >= len(odd_days):
    f.writelines(str("YES"))
  else:
    f.writelines(str("NO"))
